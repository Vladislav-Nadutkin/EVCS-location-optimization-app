...
#формулировка задачи -- функция F, которую требуется максимизировать
def F(w, y):
    M = len(w)
    sum = 0
    for i in range(M):
...
#вычисление матрицы D -- матрица дистанций
...
#инициализация X и заполнение X0 -- вектор зарядных станций
def init(M, n):
...
#Метод отжига -- 1-ый эвристический алгоритм
...
#Генетический алгоритм -- 2-ой эвристический алгоритм
...
m = folium.Map([59.942644, 30.251399], zoom_start=13)
...
plt.suptitle('Василеостровский район, CS = 50, R = 500, T0 = 10000000000000', weight = 'bold')
pylab.subplot(1, 2, 1)
...
num = 5
l = [(3 + i) for i in range(num)]
p = [(0.1 + i*0.1) for i in range(num)]
k_s = [4-i for i in range(num)]
elapsed_time = [0 for i in range(num)]
target_value = [0 for i in range(num)]
elapsed_mean = 0
target_mean = 0
...
#графики
fig = plt.figure(figsize = (14.0, 5.0))
plt.suptitle('Василеостровский район, CS = 50, R = 500, L = 12, P = 0.9', weight = 'bold')
pylab.subplot(1, 2, 1)
...
for i in range(num):
    ...
    print('Value: ', target_value[i], 'Elapsed time: ', elapsed_time[i], 'T0: ', T0, 'k_max: ', k_max)
    elapsed_mean = 0
    target_mean = 0
    table_POI[0][0] = elapsed_time[0]
    table_POI[0][1] = target_value[0]
...
for i in range(num):
    ...
    elapsed_time[i] = elapsed_mean / 10
    target_value[i] = target_mean / 10
    print('Value: ', target_value[i], 'Elapsed time: ', elapsed_time[i], 'P: ', P, 'k: ', k_s[i], 'l: ', L)
    elapsed_mean = 0
    target_mean = 0
