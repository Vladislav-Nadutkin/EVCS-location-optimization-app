...
for i in range(num):
    ...
    print('Value: ', target_value[i], 'Elapsed time: ', elapsed_time[i], 'T0: ', T0, 'k_max: ', k_maxs[i])
    elapsed_mean = 0
    target_mean = 0
...
for i in range(num):
    for j in range(10):
        s_t = time.perf_counter()
        ga = genetic_algorithm(L, P, k_s[i])
        e_t = time.perf_counter() - s_t
        elapsed_mean += e_t
        target_mean += ga[1]
    elapsed_time[i] = elapsed_mean / 10
...
#графики
fig = plt.figure(figsize = (14.0, 5.0))
plt.suptitle('MO Финляндский округ, CS = 10, R = 500, L = 12, P = 0.9', weight = 'bold')
pylab.subplot(1, 2, 1)
plt.plot(list(reversed(k_s)), elapsed_time, 'r')
plt.xlabel('Граница вырождения')
plt.ylabel('Среднее время работы алгоритма')
plt.grid(True)
pylab.subplot(1, 2, 2)
plt.plot(list(reversed(k_s)), target_value, 'r')
plt.xlabel('Граница вырождения')
plt.ylabel('Среднее значение целевой функции')
plt.grid(True)
plt.show()
...
value_dict = ['Cреднее время SA', 'Среднее значение SA', 'Cреднее время GA', 'Среднее значение GA']
